//! Compound-assignment folding (`x = x + y` -> `x += y`), increment and
//! decrement, and the empty-accept branch flip with its NaN gate on float
//! ordered comparisons.
...
fn no_increment_for_vector_plus_one() {
    // `v++` is not valid WGSL for vectors.
    let out = compact(
        r#"
        @group(0) @binding(0) var<storage, read_write> buf: array<i32>;
