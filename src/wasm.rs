...
/// Panic text to `console.error`: under `panic = "abort"` the hook still runs
/// first, so browsers see the message instead of `RuntimeError: unreachable`.
/// Hand-rolled; `console_error_panic_hook` adds only mangled wasm frames.
#[wasm_bindgen(start)]
...
/// `undefined` and `null` both read as absent.
...
// Present-but-mistyped values are errors, never silent defaults
// (`{ mangle: "false" }` must not mean "unset").
...
/// Exact integers up to 2^53 only; no silent clamp.
...
/// One slot: `"full"` / `N` (= `{ decimalPlaces: N }`) / `{ decimalPlaces }` /
/// `{ significantFigures | sigFigs }`; `field_path` names the slot in errors.
fn parse_precision_mode(val: &JsValue, field_path: &str) -> Result<PrecisionMode, JsError> {
    ...
    // An array is an object with no slot keys and would read as `Full`.
...
/// `floatPrecision`: one mode for every float kind, or per-type
/// `{ f16 | f32 | f64 | abstractFloat }` slots.
fn parse_float_precision(config: &JsValue) -> Result<FloatPrecision, JsError> {
    ...
    if val.is_object() {
        ...
        if f16.is_some() || f32.is_some() || f64.is_some() || abs.is_some() {
            // Uniform keys next to per-type ones would be silently dropped.
...
/// Unknown fields are ignored so new options never break older callers.
...
/// Crate version baked into the bundle.
#[wasm_bindgen(skip_typescript)]
