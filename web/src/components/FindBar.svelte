<script lang="ts">
  ...
  let { focusTrigger, inputContent, outputContent, onclose }: Props = $props();
  ...
  function cycleScope() {
    const order: ("all" | "input" | "output")[] = ["all", "input", "output"];
    searchScope = order[(order.indexOf(searchScope) + 1) % order.length];
  }

  // focusTrigger is read only so each open re-runs this.
  $effect(() => {
    void focusTrigger;
    inputEl?.focus();
    inputEl?.select();
  ...
  $effect(() => {
    void inputContent;
    void outputContent;
    if (contentDebounceTimer !== undefined) clearTimeout(contentDebounceTimer);
  ...
  $effect(() => {
    ...
    // A bad pattern must clear the results, not throw inside the walk.
    ...
    for (const container of containers) {
      // Shiki splits tokens across text nodes: match on the joined text and
      // map offsets back.
      const walker = document.createTreeWalker(
        container,
        NodeFilter.SHOW_TEXT,
        (node) =>
          (node.parentElement as Element)?.closest("[data-placeholder]")
      ...
      // Plain queries go through an escaped regex too: lowercasing the
      // haystack would shift offsets wherever case folding changes length.
      const matches: { start: number; end: number }[] = [];
      const re = new RegExp(
        useRegex ? q : q.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
        reFlags,
      );
      let m;
  ...
  function applyHighlights() {
    ...
    try {
      ...
      // The DOM changed under the ranges.
      clearHighlights();
      allRanges = [];
      matchCount = 0;
      currentIndex = -1;
...
<svelte:head>
  <!-- eslint-disable-next-line svelte/no-at-html-tags -- static CSS; lightningcss would drop ::highlight() -->
  {@html `<style>
...
>
    ...
    <div class="flex items-center gap-px pr-1">
      ...
      <button
        ...
        'all'
          ? 'text-emerald-400 bg-emerald-400/15'
          : 'text-slate-500 hover:text-slate-300 hover:bg-white/8'}"
        title="Search scope: {scopeLabels[searchScope]} (click to cycle)"
      >
  ...
  <div class="flex items-center gap-px">
    <button
      onclick={goPrev}
      class="p-1 rounded transition-colors cursor-pointer {matchCount === 0
        ? 'text-slate-600'
